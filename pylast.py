...
ImageSizes = collections.namedtuple(
    "ImageSizes", [
        ...
        "small", "medium", "large", "extralarge", "mega", "original"])
...
class _BaseObject(object):
    ...
    def _get_things(
        ...
        for node in doc.getElementsByTagName(thing):
            ...
            image_list = _extract_with_attribute(node, "image", "size")
            images = ImageSizes(*[image_list.get(key, None) for key in ImageSizes._fields])

            seq.append(TopItem(
                thing_type(artist, title, self.network, images=images), playcount))
...
class _Opus(_BaseObject, _Taggable):
    ...
    def __init__(self, artist, title, network, ws_prefix, username=None, **kwargs):
        """
    ...
    def playcount(self):
        try:
            ...
            self._playcount = self.get_playcount()
            return self._playcount
...
class Album(_Opus):
    ...
    def __init__(self, artist, title, network, username=None, **kwargs):
        super(Album, self).__init__(artist, title, network, "album", username, **kwargs)
    ...
    def get_cover_image(self, size=COVER_EXTRA_LARGE, force=False):
        """
        ...
        size can be one of:
            ...
            COVER_MEDIUM
            COVER_SMALL
        When retrieveing top items last.fm sends less images than
        using getInfo, then we can use the force argument
        """

        if self.images and not force:
            return self.images[size]
        image_list = _extract_with_attribute(
            self._request(
                self.ws_prefix + ".getInfo", cacheable=True), 'image', 'size')
        self.images = ImageSizes(*[image_list.get(key, None) for key in ImageSizes._fields])
        return self.images[size]
...
class Artist(_BaseObject, _Taggable):
    ...
    def get_cover_image(self, size=COVER_MEGA, force=False):
        """
        Returns a uri to the cover image
        size can be one of:
            COVER_MEGA
...
#            self._request(self.ws_prefix + ".getInfo", True), "image")[size]
        ...
        image_list = _extract_with_attribute(
            self._request(
                self.ws_prefix + ".getInfo", cacheable=True), 'image', 'size', overwrite=False)
        self.images = ImageSizes(*[image_list.get(key, None) for key in ImageSizes._fields])
        return self.images[size]
...
class Library(_BaseObject):
    ...
    def get_tracks(self, limit=50, cacheable=True):
        """
        Returns a sequence of Album objects
        If limit==None it will return all (may take a while)
        """
...
#                self.ws_prefix + ".getRecentTracks",
                ...
                params):
            name = _extract(node, "name")
            artist = _extract(node, "artist")
            album = _extract(node, "album")
            playcount = _number(_extract(node, "playcount"))
            tagcount = _number(_extract(node, "tagcount"))

            seq.append(LibraryItem(
                Track(artist, name, self.network, album=album), playcount, tagcount))

        return seq

    def oldget_tracks(self, artist=None, album=None, limit=50, cacheable=True):
        """
        Returns a sequence of Album objects
        If limit==None it will return all (may take a while)
        """
...
class Track(_Opus):
    ...
    def __init__(self, artist, title, network, username=None, album_title=None, **kwargs):
        super(Track, self).__init__(artist, title, network, "track", username, **kwargs)
        if album_title is not None:
            self._album_title = album_title

    @property
    def album_object(self):
        try:
            return self._albumObject
        except:
            self._albumObject = self.get_album()
            return self._albumObject

    @album_object.setter
    def album_object(self, album):
        self._albumObject = album
    ...
    def album_title(self):
        try:
            ...
            return self.album_object.title
    ...
    def get_position(self):
        """Returns the track position in the album"""

        doc = self._request(self.ws_prefix + ".getInfo", True)

        try:
            return doc.getElementsByTagName('album')[0].getAttribute("position")
        except:
            return 0
...
class User(_BaseObject, _Chartable):
    ...
    def get_recent_tracks_yield(self, limit=10, cacheable=True,
                          time_from=None, time_to=None, page_limit=None):
        """
        Returns this user's played track as a sequence of PlayedTrack objects
        in reverse order of playtime, all the way back to the first track.
...
def _collect_nodes_yield(limit, sender, method_name, cacheable, params=None):
    """
    Returns a sequence of dom.Node objects about as close to limit as possible
    """
    ...
    while not end_of_pages and (not limit or (limit and count < limit)):
        params["page"] = str(page)
        doc = sender._request(method_name, cacheable, params)
        doc = cleanup_nodes(doc)
...
def _extract_with_attribute(node, name, attr, overwrite=True):
    """Extracts nodes with matching attribute and returns
     a dict with {attr_value: node_value}.
     Elements with duplicate attr=value are overwritten
    ...
    for element in nodes:
        if not element.hasAttribute(attr) and not element.firstChild:
            continue
        attr_value = element.getAttribute(attr)
        if attr_value in element_dict and not overwrite:
            continue
        if not element.firstChild:
            continue
        node_value = _unescape_htmlentity(element.firstChild.data.strip())
        element_dict[attr_value] = node_value
    return element_dict
